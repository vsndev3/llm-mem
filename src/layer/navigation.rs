...
impl LayerNavigator {
    ...
    fn trace_abstraction_sources<'a>(
        &'a self,
        memory_id: &'a str,
        levels: usize,
            ...
            for src_id in &start_memory.metadata.abstraction_sources {
                if let Ok(Some(mem)) = self.memory_manager.get(&src_id.to_string()).await {
                    if levels == 1 {
                        ...
                        let deeper_sources = self
                            .trace_abstraction_sources(&src_id.to_string(), levels - 1)
                            .await?;
    ...
    pub async fn get_layer_stats(&self) -> Result<LayerStats> {
        ...
        for memory in &all_memories {
            ...
            let layer_count = stats
                .by_layer
                .entry(level)
                .or_insert_with(LayerCount::default);
