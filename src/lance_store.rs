...
use crate::types::{
    DerivedEntry, Filters, Memory, MemoryMetadata, MemoryType,
    RelationEntry, ScoredMemory,
};
...
fn build_filter_expression(filters: &Filters) -> Result<Option<String>> {
    ...
    if let Some(ref user_id) = filters.user_id {
        expressions.push(format!("metadata_json LIKE '%\"user_id\":\"{}\"%'", user_id.replace('\'', "''")));
    }
    
    if let Some(ref agent_id) = filters.agent_id {
        expressions.push(format!("metadata_json LIKE '%\"agent_id\":\"{}\"%'", agent_id.replace('\'', "''")));
    }
    
    if let Some(ref entities) = filters.entities {
        for entity in entities {
            expressions.push(format!("metadata_json LIKE '%{}%'", entity.replace('\'', "''")));
        }
    }
    
    if let Some(ref topics) = filters.topics {
        for topic in topics {
            expressions.push(format!("metadata_json LIKE '%{}%'", topic.replace('\'', "''")));
        }
    }
    ...
    if let Some(ref relations) = filters.relations {
        for relation in relations {
            ...
            expressions.push(format!("metadata_json LIKE '%\"relation\":\"{}\"%'", relation_str));
        }
    }
...
impl LanceDBStore {
        ...
        let table = match db.open_table(&config.table_name).execute().await {
            ...
            Err(_) => {
                ...
                db.create_table(&config.table_name, batches)
                    .execute()
                    .await
                    .map_err(|e| MemoryError::VectorStore(format!("LanceDB table creation failed: {e}")))?
            }
        };
    ...
    fn batch_row_to_memory(batch: &RecordBatch, row: usize) -> Result<Memory> {
        ...
        let content_meta_array = batch.column(4).as_any().downcast_ref::<StringArray>().unwrap();
        let derived_data_array = batch.column(5).as_any().downcast_ref::<StringArray>().unwrap();
        let relations_array = batch.column(6).as_any().downcast_ref::<StringArray>().unwrap();
        let context_embeddings_array = batch.column(7).as_any().downcast_ref::<StringArray>().unwrap();
        let relation_embeddings_array = batch.column(8).as_any().downcast_ref::<StringArray>().unwrap();
        let created_at_array = batch.column(9).as_any().downcast_ref::<StringArray>().unwrap();
        let updated_at_array = batch.column(10).as_any().downcast_ref::<StringArray>().unwrap();
        ...
        let metadata = serde_json::from_str(
            if metadata_array.is_null(row) { "{}" } else { metadata_array.value(row) }
        ).unwrap_or_else(|_| MemoryMetadata::new(MemoryType::Conversational));

        let content_meta: crate::types::ContentMeta = serde_json::from_str(
            if content_meta_array.is_null(row) { "{}" } else { content_meta_array.value(row) }
        ).unwrap_or_default();

        let derived_data: std::collections::HashMap<String, DerivedEntry> = serde_json::from_str(
            if derived_data_array.is_null(row) { "{}" } else { derived_data_array.value(row) }
        ).unwrap_or_default();

        let relations: std::collections::HashMap<String, RelationEntry> = serde_json::from_str(
            if relations_array.is_null(row) { "{}" } else { relations_array.value(row) }
        ).unwrap_or_default();
        ...
        let context_embeddings: Option<Vec<Vec<f32>>> = if context_embeddings_json == "[]" || context_embeddings_json.is_empty() {
            None
        } else {
        ...
        let relation_embeddings: Option<Vec<Vec<f32>>> = if relation_embeddings_json == "[]" || relation_embeddings_json.is_empty() {
            None
        } else {
...
impl crate::vector_store::VectorStore for LanceDBStore {
    async fn insert(&self, memory: &Memory) -> Result<()> {
        ...
        let content_meta_json = serde_json::to_string(&memory.content_meta)
            .map_err(|e| MemoryError::VectorStore(format!("ContentMeta serialization failed: {e}")))?;
        let derived_data_json = serde_json::to_string(&memory.derived_data)
            .map_err(|e| MemoryError::VectorStore(format!("DerivedData serialization failed: {e}")))?;
        let relations_json = serde_json::to_string(&memory.relations)
            .map_err(|e| MemoryError::VectorStore(format!("Relations serialization failed: {e}")))?;
        let context_embeddings_json = serde_json::to_string(&memory.context_embeddings)
            .map_err(|e| MemoryError::VectorStore(format!("Context embeddings serialization failed: {e}")))?;
        let relation_embeddings_json = serde_json::to_string(&memory.relation_embeddings)
            .map_err(|e| MemoryError::VectorStore(format!("Relation embeddings serialization failed: {e}")))?;
        ...
        let batch = RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(list_array),
                Arc::new(StringArray::from(vec![memory.content.clone().unwrap_or_default()])),
    ...
    async fn search(
        ...
        let mut query = self.table.query().nearest_to(query_vector).unwrap().limit(limit);
        ...
        while let Some(batch_result) = stream.next().await {
            let batch = batch_result.map_err(|e| MemoryError::VectorStore(format!("Failed to get batch: {e}")))?;
    ...
    async fn search_with_threshold(
        ...
        if let Some(threshold) = score_threshold {
            Ok(results.into_iter().filter(|item| item.score >= threshold).collect())
        } else {
            Ok(results)
        }
    }
    ...
    async fn delete(&self, id: &str) -> Result<()> {
        ...
        self.table.delete(&format!("id = '{escaped_id}'"))
            .await
    ...
    async fn count(&self) -> Result<usize> {
        let count = self.table.count_rows(None)
            .await
...
mod tests {
    ...
    async fn test_insert_and_list() {
        ...
        store.insert(&create_test_memory("test-1", "First memory")).await.unwrap();
        store.insert(&create_test_memory("test-2", "Second memory")).await.unwrap();
        store.insert(&create_test_memory("test-3", "Third memory")).await.unwrap();
    ...
    async fn test_count() {
        ...
        store.insert(&create_test_memory("test-1", "First")).await.unwrap();
        store.insert(&create_test_memory("test-2", "Second")).await.unwrap();
    ...
    async fn test_delete() {
        ...
        store.insert(&create_test_memory("test-1", "To delete")).await.unwrap();
    ...
    async fn test_candidate_ids_filter() {
        ...
        store.insert(&create_test_memory("mem-1", "First")).await.unwrap();
    ...
    async fn test_limit_and_offset() {
        ...
        for i in 1..=5 {
            store.insert(&create_test_memory(&format!("test-{}", i), &format!("Memory {}", i))).await.unwrap();
        }
