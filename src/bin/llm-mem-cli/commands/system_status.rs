...
pub(crate) fn determine_readiness(llm_available: bool, embedding_available: bool, state: &str) -> (&'static str, bool) {
    let ready = llm_available && embedding_available && state == "ready";
    if ready {
        ("READY — System is fully operational. You can store and query memories.", true)
    } else if state == "initializing" {
        ("PREPARING — Models are loading or downloading. Please wait and call system_status again in a few seconds.", false)
    } else {
        ("NOT READY — System encountered an error during initialization. Check 'last_error' for details.", false)
    }
}

/// Handle the system-status command
pub async fn handle_system_status(system: &System, format: OutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut by_layer: std::collections::HashMap<usize, (u64, u64, u64, u64, u64)> = std::collections::HashMap::new();
        ...
        if let Ok(bank) = system.bank_manager.get_or_create(&bank_info.name).await
            && let Ok(memories) = bank.list(&Filters::new(), None).await {
                for memory in &memories {
                    ...
                    entry.0 += 1; // count
                    match state {
                        "active" => { entry.1 += 1; state_counts[0] += 1; }
                        "forgotten" => { entry.2 += 1; state_counts[1] += 1; }
                        "processing" => { entry.3 += 1; state_counts[2] += 1; }
                        "invalid" => { entry.4 += 1; state_counts[3] += 1; }
                        _ => {}
                    }
    ...
    guide.insert("readiness_message".to_string(), serde_json::json!(readiness));
    guide.insert("system_status".to_string(), serde_json::to_value(&llm_status)?);
    
    guide.insert("disk_usage".to_string(), serde_json::json!({
    ...
    guide.insert("active_banks".to_string(), serde_json::json!({
        "count": bank_count,
        "names": bank_names,
    }));

    guide.insert("layer_statistics".to_string(), serde_json::json!({
    ...
    guide.insert("abstraction_pipeline".to_string(), serde_json::json!({
