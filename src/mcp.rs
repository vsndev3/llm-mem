...
fn build_usage_guide() -> serde_json::Value {
    json!({
        ...
        "tips": [
            ...
            "Search scores reflect semantic similarity, NOT whether the answer is present. Always verify retrieved content against your question — high similarity does not mean the answer exists in that memory.",
            "L0 is concrete source content (facts, verbatim text). L1+ layers are increasingly abstract (summaries → concepts → wisdom). Use L1+ memories to NAVIGATE to relevant L0 facts, but the answer itself lives in L0.",
            "Use k >= 5 when searching. Small k risks missing the right memory; larger k gives enough coverage to find and verify the correct fact.",
