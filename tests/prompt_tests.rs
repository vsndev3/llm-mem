...
fn test_quick_start_no_args() {
    ...
    let has_user_msg = result.messages.iter().any(|m| matches!(m.role, rmcp::model::PromptMessageRole::User));
    let has_assistant_msg = result.messages.iter().any(|m| matches!(m.role, rmcp::model::PromptMessageRole::Assistant));
...
fn test_build_memory_graph_no_args() {
    let result = prompts::get_prompt("build_memory_graph", None).expect("build_memory_graph should succeed");
    assert!(result.messages.len() >= 3, "Should have at least 3 messages (user/assistant/user)");
    assert!(result.description.is_some());
}
...
fn test_build_memory_graph_with_topic_and_bank() {
    ...
    assert!(content.contains("machine learning"), "Should contain the topic");
...
fn test_auto_process_document_with_file_path() {
    ...
    let content = serde_json::to_string(&result.messages).unwrap();
    assert!(content.contains("/tmp/test.pdf"), "Should contain the file path");
...
fn test_guided_document_ingest() {
    ...
    let result = prompts::get_prompt("guided_document_ingest", Some(&args)).expect("should succeed");
    assert!(result.messages.len() >= 3);

    let content = serde_json::to_string(&result.messages).unwrap();
    assert!(content.contains("/tmp/paper.pdf"));
    assert!(content.contains("STOP"), "Should instruct to stop and review");
    assert!(content.contains("Extract facts"), "Should mention fact extraction");
    assert!(content.contains("Link related"), "Should mention linking");
    assert!(content.contains("summary cards"), "Should mention summaries");
}
...
fn test_explore_knowledge_with_query() {
    ...
    let content = serde_json::to_string(&result.messages).unwrap();
    assert!(content.contains("neural networks"), "Should contain the query");
    assert!(content.contains("5"), "Should contain the depth");
    assert!(result.description.as_ref().unwrap().contains("neural networks"));
}

#[test]
fn test_explore_knowledge_default_values() {
    let result = prompts::get_prompt("explore_knowledge", None).expect("should succeed with no args");
    let content = serde_json::to_string(&result.messages).unwrap();
    assert!(content.contains("general knowledge"), "Should use default query");
...
fn test_prompt_definitions_have_correct_arguments() {
    ...
    let auto = prompts.iter().find(|p| p.name == "auto_process_document").unwrap();
    ...
    let explore = prompts.iter().find(|p| p.name == "explore_knowledge").unwrap();
    ...
    assert!(quick.arguments.is_none(), "quick_start should have no arguments");
}
...
fn test_required_arguments_marked_correctly() {
    ...
    assert_eq!(file_path_arg.required, Some(true), "file_path should be required");
...
fn test_all_prompt_messages_have_valid_roles() {
    for prompt_name in &["quick_start", "build_memory_graph", "auto_process_document", "guided_document_ingest", "explore_knowledge"] {
        let result = prompts::get_prompt(prompt_name, None)
            .unwrap_or_else(|_| panic!("{} should succeed", prompt_name));

        for msg in &result.messages {
            match msg.role {
                rmcp::model::PromptMessageRole::User | rmcp::model::PromptMessageRole::Assistant => {}
            }
        }
    }
}
